...
---@alias mega.cmdparse.Action
---    | "append"
---    | "count"
---    | "store_false"
---    | "store_true"
---    | fun(data: mega.cmdparse.ActionData): nil
...
---@class mega.cmdparse._BaseSubparsersOptions
...
---@class mega.cmdparse.SubparsersOptions: mega.cmdparse._BaseSubparsersOptions
---    Customization options for the new cmdparse.Subparsers.
---@field name string
---    The identifier for all parsers under this instance.

---@class mega.cmdparse.SubparsersInputOptions: mega.cmdparse._BaseSubparsersOptions
---    Customization options for the new cmdparse.Subparsers.
---@field [1] string?
...
local _LOGGER = logging.get_logger("mega.cmdparse._cli.cmdparse")
...
_P.Subparsers.__index = _P.Subparsers
...
local function _is_listicle(parameter)
    ...
    if type(nargs) ~= "number" then
        ...
        error(string.format('Bug: Unknown "%s" nargs was found. Please fix!', nargs), 0)
    end
...
local function _get_used_position_arguments_count(position, arguments)
    ...
    return _P.validate_variable_position_arguments(nargs, arguments, position.names[1])
end
...
---
function _P.validate_variable_position_arguments(nargs, arguments, name)
    local found = 0
...
function _P.Parameter:is_position()
...
---
function _P.Parameter:get_type()
    return self._type
end
...
---
function _P.Parameter:increment_used(increment)
...
---
function _P.Parameter:set_nargs(count)
...
---@param item mega.cmdparse.ParameterParser | mega.cmdparse.Parameter
---@param parent_parser mega.cmdparse.ParameterParser
---@param argument argparse.Argument
---@param display_options mega.cmdparse._core.DisplayOptions
---@param contexts mega.cmdparse.ChoiceContext[]
---    A description of how / when this function is called. It gets passed to
---    `cmdparse.Parameter.choices()`.
---@return string[]
function _P.get_partial_matches(item, parent_parser, argument, display_options, contexts)
    ---@type string[]
    local output = {}
    local argument_name = text_parse.get_argument_name(argument)

    if _is_parser(item) then
        local last_value = text_parse.get_argument_value_text(argument)
        ---@cast item mega.cmdparse.ParameterParser
        vim.list_extend(output, matcher.get_parser_exact_or_partial_matches(item, argument_name, last_value, contexts))
    elseif text_parse.is_incomplete_named_argument(argument) then
        ---@cast item mega.cmdparse.Parameter
        local parameter = _get_next_parameter_if_needed(parent_parser, item, argument)
        vim.list_extend(output, _get_named_argument_completion_choices(parameter, argument, contexts))
    elseif _is_parameter(item) then
        ---@cast item mega.cmdparse.Parameter
        local parameter = _get_next_parameter_if_needed(parent_parser, item, argument)
        ...
        vim.list_extend(
            output,
            matcher.get_exact_or_partial_matches(parameter, argument, parent_parser, contexts, display_options)
        )
    else
        error(string.format('Bug found. Item "%s" is unknown.', vim.inspect(item)))
    end

    if parent_parser:is_satisfied() then
        for parser in iterator_helper.iter_parsers(parent_parser) do
            vim.list_extend(output, texter.get_array_startswith(parser:get_names(), argument_name))
        end
    end
...
function M.ParameterParser:_get_argument_usage_summary(data, parser)
    ...
    local summary = _P.get_usage_summary(parser)

    return summary, parser
end

--- Check for any startswith / partial matches from `text`, according to `options`.
---
---@param text string
---    Some raw user input to handle.
---@param options mega.cmdparse._core.DisplayOptions?
---    Control minor behaviors of this function. e.g. What data to show.
---@return string[]
---    The found partial matches, if any.
---
function M.ParameterParser:_get_empty_arguments_partial_matches(text, options)
    ---@type string[]
    local output = {}

    local data = argparse.parse_arguments(text)
    local contexts = { constant.ChoiceContext.auto_completing }

    if not vim.tbl_isempty(data.arguments) then
        ...
        then
            local named_argument = data.arguments[1]
            local value = named_argument.value
            ---@cast value string

            return matcher.get_matching_partial_flag_text(
                named_argument.name,
    ...
    if not texter.is_whitespace(text) then
        vim.list_extend(
            output,
    ...
    vim.list_extend(output, matcher.get_current_parser_completions(self, options))
...
function M.ParameterParser:_get_completion(data, column, options)
    ...
    if vim.tbl_isempty(stripped.arguments) then
        return self:_get_empty_arguments_partial_matches(remainder, display_options)
    end
    ...
    if is_allowed_to_match_partial_results then
        return _P.get_partial_matches(recent_item, parser, last, display_options, contexts)
    end
    ...
    for _, flag in ipairs(flags) do
        if vim.tbl_contains(flag.names, argument.name) and not flag:is_exhausted() then
            ...
            if needs_a_value then
                if values == nil then
                    error(string.format('Parameter "%s" requires a value. Got nothing.', argument.name), 0)
                end
            end
...
function M.ParameterParser:add_parameter(options)
    ...
    local new_options = vim.tbl_deep_extend("force", options, { parent = self })
    local parameter = _P.Parameter.new(new_options)
...
function M.ParameterParser:add_subparsers(options)
    local new_options = vim.tbl_deep_extend("force", options, { parent = self })
    local subparsers = _P.Subparsers.new(new_options)
